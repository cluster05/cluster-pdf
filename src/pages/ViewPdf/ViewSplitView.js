...
const ViewSplitView = ({ downloadUrl, documentId }) => {
  ...
  const processSplitPdf = async () => {
    ...
    try {
      const response = await documentInstance.post("/split", {
        url: downloadUrl,
        pages,
        mongoId: documentId,
      });
    ...
    <div className="flex flex-col items-center ">
      {isProcessingStart ? (
        ...
        <button
          ...
          className="px-5 py-2 rounded shadow text-xl font-semibold text-white bg-blue-400 hover:bg-blue-300 flex items-center"
        >
          Start spliting
        </button>
      )}
      <div className="mt-10">
        ...
        <section id="pdf-section" className="flex flex-col items-center w-100">
          <Document
            file={downloadUrl}
